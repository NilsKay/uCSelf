 ...
 */
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if (o == model.cb[model.USENOTES]) { 
		    ...
		    this.model.cb[model.USENOTES].setEnabled(this.def.tonal);
		    if (this.def.tonal) {
		...
		for (int n = 0; n < flow.size(); n++) {
		   ...
		   // sleep = (int) (maxd * 1000.0);
		    ...
		    for ( int ci = 0; ci < v.size(); ci++) {
				...
				if (ci == 0) {
					addLog("");
					addLog("--------> New interval n="+n+" from start="+format.format(nt.start*1000)+" ms, \tIntervaldauer="+intervall+" ms ");
				}
				...
				mid.createShortEvent(mid.NOTEON + channel, midN.midiIndex, mid.channels[channel]);
...
class MidiNote implements ActionListener {
	...
	*/
	public void startTimer(int sleep) {
    	...
    	addLog("              -> Start Channel "+this.midiChannel+"\t note="+midiIndex+" duration:"+sleep+"ms at:"+tStart+" Systemtime");
		this.isOn = true;
		activeNotes++;
	}
