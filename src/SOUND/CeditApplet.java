...
public class CeditApplet extends Applet implements Runnable {
	...
	label.setText("Found class:"+name);
        if ((name == null) || name.equals("java.lang.Object")) {
            ...
            while (windowThread != null) {
                ...
                pleaseCreate = false;
                Frame window = null;
		label.setText("Try do instance this class");
                try {
                    //window = (Frame)windowClassObject.newInstance();
		    window = (Frame) new CEditor(Home, User, options);
		    //((CEditor) window).setEnviroment(Home, User);
                } catch (Exception e) {
