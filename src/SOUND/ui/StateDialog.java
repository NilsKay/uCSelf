...
public class StateDialog extends JDialog {
	...
	public StateDialog(CEditor ce, String title) {
	...
	class StateModel {
		...
		public StateModel() {
			...
			cb[USEDELAY] = new JCheckBox(ce.def.text[133]);
			cb[USEDELAY].setSelected(ce.def.doDelay);
		...
		public void save() {
			...
			ce.def.doDelay = cb[USEDELAY].isSelected();
	...
	class StateUi extends JPanel{
		...
		private JComponent do1stLayout() {
	        ...
	        builder.add(model.cb[model.USEDELAY],		cc.xy (7,  row));
	        row += 2;
